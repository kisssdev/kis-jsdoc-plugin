...
## Using TypeScript to describe type

JSDoc uses its **own syntax** or **Google closures** to describe types. Unfortunately it does not really support TypeScript although Visual Studio Code is able to infer type with JSDoc TypeScript definitions.
This plugin enables to reconstruct valid documentation from TypeScript definitions even if the JSDoc CLI will complain with following errors: 
...
Note that the following comments formats are supported for functions:
 ...
 ```javascript
   ...
   * @param {(user: User) => void} p1 parameter description
   * @return {Promise<{ name: string }>} return description
   */
   function f1(p1) { 
    ...
   }
...
or
  ...
  /**
   * Function description.
   * @param {(user: User) => void} parameter1 parameter description
   * @return {Promise<{ name: string }>} return description
   */
  const f1 = p1 => {
    ...
 ...
 ```javascript
  /** @type {() => {name: string}} the member description */ m1;
